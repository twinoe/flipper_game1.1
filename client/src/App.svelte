<script lang="ts">
  ...
  let highscores: Array<{ score: number }> = [];
  ...
  async function sendScore(scoreValue: number) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Fehler beim Speichern des Scores');
  ...
  sendScore(score);  // Score an Backend senden

  score = 0;           // Score zurücksetzen fürs nächste Spiel
...
<div class="score-board">
  ...
  <div>Aktueller Score: {score}</div>

  <h3>Highscores:</h3>

  {#each highscores as highscore, i}

    <div>{i + 1}. {highscore.score}</div>
